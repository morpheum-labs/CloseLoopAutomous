...
**Backlog checklist (§1–§10):** `89` done · `17` open · **~84%** complete — see **[Master backlog (all checklist items)](#master-backlog-all-checklist-items)** for the full table; _grep_ `- [x]` / `- [ ]` in this file to refresh counts after edits._
...
_Re-checked against the `arms/` tree (2026-03-23): SQLite schema **v21** (`ExpectedSchemaVersion` in `internal/adapters/sqlite/migrate.go`); baseline vs “full MC” is called out so unchecked rows are not misread as “missing entirely” when a slim table or route already exists._
...
**Next vertical slices (suggested):** **(1)** ML / preference pipeline consuming **`swipe_history`** + **`preference_models`**. **(2)** Convoy **graph algorithms + richer mailbox** (cross-agent). **(3)** Optional **`/api/openclaw/*`** HTTP proxy if the UI needs it. **(4)** Broaden **operations_log** coverage + operator filters. **(5)** Optional PR idempotency keys (#60 polish).
