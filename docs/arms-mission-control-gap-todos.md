...
**Backlog checklist (§1–§10):** `97` done · `11` open · **~90%** complete — see **[Master backlog (all checklist items)](#master-backlog-all-checklist-items)** for the full table; _grep_ `- [x]` / `- [ ]` in this file to refresh counts after edits._

**Next priority:** **Knowledge** — **auto-ingest** from **product_feedback** / swipes / agent completion summaries (**CRUD + dispatch injection + optional `chromem` backend done**). **Convoy:** **`dominikbraun/graph`** + **`GET /api/convoys/{id}/graph`** **done**; **MC-exact convoy API/fields** still **#70** / **#66**. **Stalled-task reassign** policy still **TBD** when task↔agent binding exists (**#93**/**#94**). **Preference:** **embeddings / trained model** on **`preference_models`** still **TBD**. Optional polish: stable PR correlation keys for extreme dedupe.
...
_Re-checked against the `arms/` tree (2026-03-24): SQLite schema **v25** (`ExpectedSchemaVersion` in `internal/adapters/sqlite/migrate.go`); baseline vs “full MC” is called out so unchecked rows are not misread as “missing entirely” when a slim table or route already exists._
...
| **B — Full autonomy** | ~2 wk | Convoy: **done (baseline DAG semantics):** `convoy_subtasks.completed` (migration 011); dependents **`dispatch-ready`** only after upstream **completed**; webhook **`convoy_id` + `subtask_id`** + parent **`task_id`**; SSE **`convoy_subtask_dispatched`** / **`convoy_subtask_completed`**. **Baseline:** parent-task **agent-health gate** + **`dispatch_attempts`** / gateway retry cap (migration **018**). **Partial (2026-03):** migration **`022`** — convoy **`metadata_json`**, subtask **`title`** / **`metadata_json`** / **`dag_layer`**, **`convoy_mail`** **`kind`** / **`from_subtask_id`** / **`to_subtask_id`**; HTTP graph summary + subtask **`status`**, enriched mail POST. **Graph slice (2026-03):** **`github.com/dominikbraun/graph`** + **`GET /api/convoys/{id}/graph`** (+ **`/api/convoy/.../graph`**) — stable **`topological_order`**, **`edges`**, **`layers`**. **TBD:** MC-exact convoy API/fields (**#70**), optional persisted edge table / full MC DAG analytics (**#66**), cross-agent mailbox unification. **GitHub PR + post-execution (#60)** — **done (baseline):** REST + **`gh`**, duplicate recovery, agent + **CI** HMAC webhooks, auto PR/merge path for tiers. Deeper **ideas** scoring/metadata; **`swipe_history`** table + list API (**done**); **`preference_models`** — **heuristic learning loop** (**aggregate** swipe + **`product_feedback`** + maybe-pool → **`preference_models`**, ideation bias, event-driven recompute + **`POST …/recompute`**) **done**; **ML / embeddings** still **TBD**. |
...
**Next vertical slices (suggested):** **(1)** ~~Convoy **`dominikbraun/graph`**~~ **done** — follow with **MC-exact convoy API/fields** + optional **`convoy_edges`** table if analytics need it. **(2)** **Knowledge auto-ingest** (feedback, swipes, completion summaries); **semantic** backend: **`ARMS_KNOWLEDGE_BACKEND=chromem`** (**`internal/adapters/chromemstore`**) shipped; **Mem0** HTTP adapter optional later. **(3)** **Stalled-task reassign** when execution-agent binding exists (**§7**). **(4)** **ML / embeddings** on **`preference_models`**. **(5)** Optional **`/api/openclaw/*`** HTTP proxy, **operations_log** breadth, PR idempotency keys (#60 polish).
...
| # | § | Status | Item |
|---:|---|:--:|------|
| 1 | 1 | Done | Optional **MC-compat alias routes** — `POST /api/convoy`, `GET /api/convoy/{id}`, `GET /api/convoy/{id}/graph`, `POST /api/convoy/{id}/dispatch-ready` → same handlers as `/api/convoys/...` |
...
| 30 | 2 | Done | Convoy **graph algorithms** — **`github.com/dominikbraun/graph`** (stable topological sort); **`GET /api/convoys/{id}/graph`** + **`GET /api/convoy/{id}/graph`**; payload **`topological_order`**, **`edges`**, **`layers`**, **`graph`** summary; OpenAPI **`ConvoyGraphResponse`**; **`internal/application/convoy/graph.go`**. MC-exact convoy **routes/fields** still **#70** |
...
| 37 | 2 | Done | **#37 MC parity slice:** soft delete (**`025`**) + **cascade** (`ports.RequireActiveProduct`, task/convoy/taskchat gates) + **optimistic PATCH product** (`SaveIfUnchangedSince`, `ErrStaleEntity`) + **`GET /api/ops/summary`**. Deeper row locks / task versioning / full ops UI → other backlog rows |
...
| 66 | 6 | Open | Persist **full** convoy DAG as in MC — **partial:** **`022`** + **`depends_on`** + domain **`ConvoySubtaskDagLayers`** + embedded **`graph`** on **`GET …/convoys/{id}`** + dedicated **`GET …/convoys/{id}/graph`** (topo/edges/layers); MC-complete **schema** (e.g. separate **`convoy_edges`** table) / analytics **TBD** |
...
| 69 | 6 | Done | Minimal HTTP — `POST /api/convoys`, `GET /api/convoys/{id}`, `GET /api/convoys/{id}/graph`, `GET /api/products/{id}/convoys`, `POST /api/convoys/{id}/dispatch-ready`; `convoy.Service.Get`, `ListByProduct`, `Graph`; `ports.ConvoyRepository.ListByProduct`; subtask **`completed`** in JSON |
| 70 | 6 | Open | API parity with MC convoy — **partial** after **`022`** + **`GET …/graph`** (metadata, mail, computed **`status`**, embedded + dedicated DAG payloads); full MC field/route parity **TBD** |
...
| 90 | 8 | Done | Knowledge base — migration **`024`** **`knowledge_entries`** + **`knowledge_fts`** (FTS5 default); **`ports.KnowledgeRepository`**; **`knowledge.Service`**; product CRUD **`/api/products/{id}/knowledge`**; OpenClaw **`KnowledgeForDispatch`** injects ranked snippets on **task** + **convoy subtask** dispatch; **`ARMS_KNOWLEDGE_DISPATCH_SNIPPETS`**, **`ARMS_KNOWLEDGE_DISABLE_DISPATCH`**; optional **`ARMS_KNOWLEDGE_BACKEND=chromem`** + **`internal/adapters/chromemstore`** (Ollama/OpenAI embedders; **`ARMS_CHROMEM_*`** env); **`UseFTSQuerySyntax`** false for semantic search paths |
...
- [x] Optional **MC-compat alias routes** — `POST /api/convoy`, `GET /api/convoy/{id}`, `GET /api/convoy/{id}/graph`, `POST /api/convoy/{id}/dispatch-ready` → same handlers as `/api/convoys/...`
...
- [x] `convoys` / `convoy_subtasks` / **`convoy_mail`** — migration **`022`** + HTTP: convoy **`metadata_json`**, subtask **`title`** / **`metadata_json`** / **`dag_layer`** (computed on create), mail **`kind`** / from / to; **`GET`/`POST …/convoys/{id}/mail**; **`GET …/convoys/{id}/graph`** (**`dominikbraun/graph`**: topo order, edges, layers) — **TBD:** MC-exact DAG spec + optional **`convoy_edges`** table + cross-agent mailbox unification
...
- [x] Product **soft delete** — migration **`025_products_deleted_at.sql`** + **`domain.Product.DeletedAt`**; **`ListAll`** / **`ByID`** exclude deleted rows; **`ListAllIncludingDeleted`**; HTTP **`DELETE /api/products/{id}`** (204), **`POST /api/products/{id}/restore`**, **`GET /api/products?include_deleted=1|true`** (JSON **`deleted_at`** when set); **`operations_log`** **`product.soft_delete`** / **`product.restore`**; SQLite binds SQL **NULL** for active rows; child tables unchanged (FK **CASCADE** still applies on hard deletes only)
- **#37 design (hexagonal / DRY / SOLID):** *Domain* — tombstone + **`ErrStaleEntity`** (optimistic conflicts). *Ports* — **`RequireActiveProduct`**, **`ProductRepository.SaveIfUnchangedSince`**, **`CountLifecycle`** (shared contract). *Application* — **`task`** helpers **`taskAndActiveProduct` / `taskWithActiveProduct` / `TaskByIDForAPI`**; **`product.PatchMetadata`** uses optimistic save; **convoy** / **taskchat** call **`RequireActiveProduct`** where tasks or products anchor writes. *Adapters* — SQLite/memory implement port methods only. *HTTP* — **`opsSummary`**, **`mapDomainErr`** maps **`stale_entity`**. *Further MC depth* (task row versioning, `SELECT FOR UPDATE`, full ops suite) → new rows / ports, not fat handlers.
- [x] **#37 remainder (cascade + concurrency + ops tooling):** soft-delete **cascade** — **`ports.RequireActiveProduct`**; **`task.Service`** gates mutating + read API paths on active product; **convoy** (**Create**, **DispatchReady**, **CompleteSubtask**) + **taskchat** (**Append**, **ListByTask**) aligned; **`CreateFromApprovedIdea`** requires active **`idea.ProductID`**; **optimistic lock** — **`SaveIfUnchangedSince`** + **`domain.ErrStaleEntity`** on **`PATCH /api/products/{id}`**; **ops** — **`GET /api/ops/summary`**, **`ProductRepository.CountLifecycle`**, **`Handlers.ExpectedSchemaVersion`** from **`sqlite.ExpectedSchemaVersion`**
...
- [x] Convoy DAG **graph algorithms** — **`github.com/dominikbraun/graph`** + **`GET /api/convoys/{id}/graph`** (+ alias): **`topological_order`**, **`edges`**, **`layers`**, summary (edges derived from **`depends_on`**; no separate edge table). **TBD:** MC-complete payloads/routes **#70**, optional persisted **`convoy_edges`** **#66**
...
- [x] Minimal HTTP — `POST /api/convoys`, `GET /api/convoys/{id}`, `GET /api/convoys/{id}/graph`, `GET /api/products/{id}/convoys`, `POST /api/convoys/{id}/dispatch-ready`; `convoy.Service.Get`, `ListByProduct`, `Graph`; `ports.ConvoyRepository.ListByProduct`; subtask **`completed`** in JSON
- [ ] API parity with MC convoy — **partial** after **`022`** + **`GET …/graph`** (metadata, mail, **`status`**, **`graph`**); full MC routes/fields **TBD** (**naming / singular aliases:** done — §1)
...
- [x] Learner / knowledge base — SQLite **FTS5** default (**`024`**), **`KnowledgeRepository`**, HTTP **`POST/GET/PATCH/DELETE …/knowledge`**, dispatch injection via **`openclaw.Options.KnowledgeForDispatch`** (task + convoy subtask); **`ARMS_KNOWLEDGE_BACKEND=chromem`** for **chromem-go** semantic search (**`ARMS_CHROMEM_*`**); disable injection with **`ARMS_KNOWLEDGE_DISABLE_DISPATCH`**; **auto-ingest** from feedback/swipes still **TBD**
...
|-----------------|--------------------------------------------------------------|
| SQLite + core tables | Unblocks everything else (current **v25** migrations)     |
