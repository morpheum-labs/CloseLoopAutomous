...
func routeCatalog() []RouteEntry {
	return []RouteEntry{
		...
		{"POST", "/api/gateway-endpoints/{id}/test-connection", "Run Mission Control connection checklist (optional JSON body { draft?: { gateway_url?, gateway_token? (omit=stored; empty=test unauthenticated), driver?, device_id?, timeout_sec? } }; returns { steps: [...] }; persists connection_status / pairing_* on OpenClaw-class ws drivers when applicable)"},
		{"DELETE", "/api/gateway-endpoints/{id}", "Remove gateway profile (409 if any execution agent references it)"},
		{"GET", "/api/fleet/identities", "List discovered AgentIdentity rows (?limit= default 200, max 500); POST /api/fleet/refresh runs remote list per gateway (OpenClaw agents.list) + optional GeoIP"},
		{"GET", "/api/fleet/identities/{id}", "Get one AgentIdentity by profile id (see domain.FleetProfileID)"},
		{"POST", "/api/fleet/refresh", "Re-synthesize and upsert all agent identities; SSE agent_identity_updated per gateway"},
		{"GET", "/api/fleet/geo-summary", "Aggregate country counts from identities with geo (maxmind_geoip2)"},
		{"GET", "/api/agents", "registry[] execution agents + items[] recent task heartbeats (stub:true on items only when agent health disabled); fleet profiles are GET /api/fleet/identities"},
		{"POST", "/api/agents", "Register execution agent {display_name, gateway_endpoint_id, session_key (required unless endpoint driver is stub), optional product_id, source, external_ref}"},
